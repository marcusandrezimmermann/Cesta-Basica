...
'C:\\Users\\marcu\\Documents\\Tese\\Script\\Bases Ifood\\ES' -> diretorio
...
Base_ES2 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                localidade = dia, dia = hora, hora = X) -> Base_ES2

Base_ES3 <- data.frame()

for(i_arq in 316:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_ES3, res) -> Base_ES3
}
...
rbind(Base_ES, Base_ES2) -> Base_total_ES
rbind(Base_total_ES, Base_ES3) -> Base_total_ES

# Ajustando a base sem dados relevantes

Base_total_ES %>%
  dplyr::filter(!(hora == "")) -> Base_total_ES_ajust

# Ajustando os dias do ano para um único padrão: DD/MM/AAAA

unique(Base_total_ES_ajust$dia) -> f
...
Base_total_ES_ajust$dia <- plyr::mapvalues(Base_total_ES_ajust$dia,
                                          from = f,
                                          to = t)
...
SOAR::Store(Base_total_ES_ajust, lib = R_Tese)
...
Base_total_ES_ajust$teste <- apply(sapply(array , grepl, Base_total_ES_ajust$nome_produto), 1, any)
...
Base_total_ES_ajust$preco_produto <- as.numeric(Base_total_ES_ajust$preco_produto)
                                                                        ...
                                                                        ifelse(produto_base == 'Farinha de trigo', preco*1.5,
                                                                               ifelse(produto_base == 'Tomate kg', preco*9,
                                                                                      ifelse(produto_base == 'Café', preco*0.6,
                                                                                             ifelse(produto_base == 'Banana kg', preco_duzia*7.5, preco*6))))))))))) -> Base_ES_reg

# Armazenando a base

SOAR::Store(Base_ES_reg, lib = R_Tese)
...
Base_ES_reg$mes <- as.character(Base_ES_reg$mes)
...
Base_ES_reg %>%
  dplyr::filter(teste == 'FALSE',
                ...
                mes != '202303',
                !(is.na(preco_cesta)),
                preco_cesta != 0) -> Base_ES_reg

Base_ES_reg$produto_base <- plyr::mapvalues(Base_ES_reg$produto_base,
...
DIEESE %>%
  dplyr::filter(UF == 'ES') %>%
  dplyr::select(-UF) %>%
  as.data.frame() -> DIEESE_ES

DIEESE_ES %>%
  dplyr::filter(produto_base == 'Total') %>%
  dplyr::select(-produto_base) -> DIEESE_ES_total

Base_ES_reg %>%
  dplyr::left_join(DIEESE_ES, by = c('produto_base', 'mes')) -> Cesta_BasicaIfood_ES

cv<-function(x){coef<-sd(x)/mean(x)*100 
return(coef)}

Cesta_BasicaIfood_ES %>%
...
# Cesta_BasicaIfood_ES %>%
#   dplyr::group_by(produto_base) %>%
#   dplyr::mutate(teste2 = ifelse(produto_base == 'Carne' & preco_cesta >= median(preco_cesta)*1.3 & preco_cesta <= median(preco_cesta)*3.2, 'FALSE',
#                                 ifelse(produto_base == 'Banana' & preco_cesta >= median(preco_cesta)*1 & preco_cesta <= median(preco_cesta)*6.8, 'FALSE',
#                                        ifelse(produto_base == 'Batata' & preco_cesta >= median(preco_cesta)*0.7 & preco_cesta <= median(preco_cesta)*3, 'FALSE',
#                                               ifelse(produto_base == 'Tomate' & preco_cesta >= median(preco_cesta)*0.5 & preco_cesta <= median(preco_cesta)*3.8, 'FALSE',
#                                                      ifelse(produto_base == 'Pão' & preco_cesta >= median(preco_cesta)*1 & preco_cesta <= median(preco_cesta)*6.4, 'FALSE',
...
#   dplyr::left_join(DIEESE_ES_total, by = 'mes') %>%
#   dplyr::mutate(diferenca = (preco_calculado - preco_oficial)/preco_oficial) -> Cesta_Final_ES
...
Cesta_BasicaIfood_ES %>%
  ...
  dplyr:: select(mes, preco_calculado, variancia, desvio_padrao, coef_var,
                 lim_inf, lim_sup, preco_oficial, diferenca)-> Cesta_Final_ES

openxlsx::write.xlsx(Cesta_Final_ES, file='Cesta_BasicaIfood_ES.xlsx')
