...
'C:\\Users\\marcu\\Documents\\Tese\\Script\\Bases Ifood\\SC' -> diretorio
...
for(i_arq in 1:length(arquivos)) {
  ...
  rbind(Base_SC, res) -> Base_SC
}

Base_SC2 <- data.frame()

for(i_arq in 220:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SC2, res) -> Base_SC2
}
...
Base_SC2 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                localidade = dia, dia = hora, hora = X) -> Base_SC2

Base_SC3 <- data.frame()

for(i_arq in 222:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SC3, res) -> Base_SC3
}
...
Base_SC4 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_SC4


Base_SC5 <- data.frame()

for(i_arq in 270:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SC5, res) -> Base_SC5
}

Base_SC6 <- data.frame()

for(i_arq in 315:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SC6, res) -> Base_SC6
}
...
Base_SC6 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_SC6

Base_SC7 <- data.frame()

for(i_arq in 317:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SC7, res) -> Base_SC7
}
...
Base_SC8 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_SC8

Base_SC9 <- data.frame()

for(i_arq in 326:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SC9, res) -> Base_SC9
}

Base_SC10 <- data.frame()

for(i_arq in 329:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SC10, res) -> Base_SC10
}
...
Base_SC10 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_SC10

Base_SC11 <- data.frame()

for(i_arq in 330:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SC11, res) -> Base_SC11
}

Base_SC12 <- data.frame()

for(i_arq in 348:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SC12, res) -> Base_SC12
}
...
Base_SC12 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_SC12

Base_SC13 <- data.frame()

for(i_arq in 349:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SC13, res) -> Base_SC13
}

Base_SC14 <- data.frame()

for(i_arq in 353:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SC14, res) -> Base_SC14
}
...
Base_SC14 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_SC14

Base_SC15 <- data.frame()

for(i_arq in 354:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SC15, res) -> Base_SC15
}

Base_SC16 <- data.frame()

for(i_arq in 357:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SC16, res) -> Base_SC16
}
...
Base_SC16 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_SC16

Base_SC17 <- data.frame()

for(i_arq in 358:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SC17, res) -> Base_SC17
}
...
rbind(Base_SC, Base_SC2) -> Base_total_SC
rbind(Base_total_SC, Base_SC3) -> Base_total_SC
rbind(Base_total_SC, Base_SC4) -> Base_total_SC
rbind(Base_total_SC, Base_SC5) -> Base_total_SC
rbind(Base_total_SC, Base_SC6) -> Base_total_SC
rbind(Base_total_SC, Base_SC7) -> Base_total_SC
rbind(Base_total_SC, Base_SC8) -> Base_total_SC
rbind(Base_total_SC, Base_SC9) -> Base_total_SC
rbind(Base_total_SC, Base_SC10) -> Base_total_SC
rbind(Base_total_SC, Base_SC11) -> Base_total_SC
rbind(Base_total_SC, Base_SC12) -> Base_total_SC
rbind(Base_total_SC, Base_SC13) -> Base_total_SC
rbind(Base_total_SC, Base_SC14) -> Base_total_SC
rbind(Base_total_SC, Base_SC15) -> Base_total_SC
rbind(Base_total_SC, Base_SC16) -> Base_total_SC
rbind(Base_total_SC, Base_SC17) -> Base_total_SC

# Ajustando a base sem dados relevantes

Base_total_SC %>%
  dplyr::filter(!(hora == "")) -> Base_total_SC_ajust

# Ajustando os dias do ano para um único padrão: DD/MM/AAAA

unique(Base_total_SC_ajust$dia) -> f
...
Base_total_SC_ajust$dia <- plyr::mapvalues(Base_total_SC_ajust$dia,
                                          from = f,
                                          to = t)
...
SOAR::Store(Base_total_SC_ajust, lib = R_Tese)
...
Base_total_SC_ajust$teste <- apply(sapply(array , grepl, Base_total_SC_ajust$nome_produto), 1, any)
...
Base_total_SC_ajust$preco_produto <- as.numeric(Base_total_SC_ajust$preco_produto)
                                                                                      ...
                                                                                      ifelse(produto_base == 'Café', preco*0.6,
                                                                                             ifelse(produto_base == 'Batata kg', preco*6,
                                                                                                    ifelse(produto_base == 'Pão francês', preco*6,
                                                                                                           ifelse(produto_base == 'Banana kg', preco_duzia*7.5, preco*6.6))))))))))))) -> Base_SC_reg

# Armazenando a base

SOAR::Store(Base_SC_reg, lib = R_Tese)
...
Base_SC_reg$mes <- as.character(Base_SC_reg$mes)
...
Base_SC_reg %>%
  dplyr::filter(teste == 'FALSE',
                ...
                mes != '202303',
                !(is.na(preco_cesta)),
                preco_cesta != 0) -> Base_SC_reg

Base_SC_reg$produto_base <- plyr::mapvalues(Base_SC_reg$produto_base,
...
DIEESE %>%
  dplyr::filter(UF == 'SC') %>%
  dplyr::select(-UF) %>%
  as.data.frame() -> DIEESE_SC

DIEESE_SC %>%
  dplyr::filter(produto_base == 'Total') %>%
  dplyr::select(-produto_base) -> DIEESE_SC_total

Base_SC_reg %>%
  dplyr::left_join(DIEESE_SC, by = c('produto_base', 'mes')) -> Cesta_BasicaIfood_SC

cv<-function(x){coef<-sd(x)/mean(x)*100 
return(coef)}

Cesta_BasicaIfood_SC %>%
...
# Cesta_BasicaIfood_SC %>%
#   dplyr::group_by(produto_base) %>%
#   dplyr::mutate(teste2 = ifelse(produto_base == 'Carne' & preco_cesta >= median(preco_cesta)*0.9 & preco_cesta <= median(preco_cesta)*2.5, 'FALSE',
#                                 ifelse(produto_base == 'Banana' & preco_cesta >= median(preco_cesta)*1.1 & preco_cesta <= median(preco_cesta)*3.4, 'FALSE',
#                                        ifelse(produto_base == 'Batata' & preco_cesta >= median(preco_cesta)*0.7 & preco_cesta <= median(preco_cesta)*2.6, 'FALSE',
#                                               ifelse(produto_base == 'Tomate' & preco_cesta >= median(preco_cesta)*0.7 & preco_cesta <= median(preco_cesta)*6.3, 'FALSE',
#                                                      ifelse(produto_base == 'Pão' & preco_cesta >= median(preco_cesta)*0.8 & preco_cesta <= median(preco_cesta)*1.9, 'FALSE',
#                                                             ifelse(produto_base %in% c('Açúcar', 'Arroz', 'Feijão', 'Farinha', 'Café', 'Óleo', 'Manteiga', 'Leite') & preco_cesta >= median(preco_cesta)*0.5 & preco_cesta <= median(preco_cesta)*2, 'FALSE', 'TRUE'))))))) %>%
#   dplyr::ungroup() %>%
#   dplyr::filter(teste2 == 'FALSE') %>%
...
#   dplyr::left_join(DIEESE_SC_total, by = 'mes') %>%
#   dplyr::mutate(diferenca = (preco_calculado - preco_oficial)/preco_oficial) -> Cesta_Final_SC
...
Cesta_BasicaIfood_SC %>%
  ...
  dplyr:: select(mes, preco_calculado, variancia, desvio_padrao, coef_var,
                 lim_inf, lim_sup, preco_oficial, diferenca) -> Cesta_Final_SC

openxlsx::write.xlsx(Cesta_Final_SC, file='Cesta_BasicaIfood_SC.xlsx')

# tmp %>%
#   dplyr::filter(mes == '202309') %>%
#   dplyr::select(produto_base) %>%
#   unique()