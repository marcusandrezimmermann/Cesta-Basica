...
'C:\\Users\\marcu\\Documents\\Tese\\Script\\Bases Ifood\\GO' -> diretorio
...
for(i_arq in 1:length(arquivos)) {
  ...
  rbind(Base_GO, res) -> Base_GO
}

Base_GO2 <- data.frame()

for(i_arq in 281:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_GO2, res) -> Base_GO2
}
...
Base_GO2 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                localidade = dia, dia = hora, hora = X) -> Base_GO2

Base_GO3 <- data.frame()

for(i_arq in 282:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_GO3, res) -> Base_GO3
}
...
Base_GO4 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_GO4
...
Base_GO6 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_GO6

Base_GO7 <- data.frame()

for(i_arq in 345:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_GO7, res) -> Base_GO7
}
...
rbind(Base_GO, Base_GO2) -> Base_total_GO
rbind(Base_total_GO, Base_GO3) -> Base_total_GO
rbind(Base_total_GO, Base_GO4) -> Base_total_GO
rbind(Base_total_GO, Base_GO5) -> Base_total_GO
rbind(Base_total_GO, Base_GO6) -> Base_total_GO
rbind(Base_total_GO, Base_GO7) -> Base_total_GO

# Ajustando a base sem dados relevantes

Base_total_GO %>%
  dplyr::filter(!(hora == "")) -> Base_total_GO_ajust

# Ajustando os dias do ano para um único padrão: DD/MM/AAAA

unique(Base_total_GO_ajust$dia) -> f
...
Base_total_GO_ajust$dia <- plyr::mapvalues(Base_total_GO_ajust$dia,
                                          from = f,
                                          to = t)
...
SOAR::Store(Base_total_GO_ajust, lib = R_Tese)
...
Base_total_GO_ajust$teste <- apply(sapply(array , grepl, Base_total_GO_ajust$nome_produto), 1, any)
...
Base_total_GO_ajust$preco_produto <- as.numeric(Base_total_GO_ajust$preco_produto)
                                                                        ...
                                                                        ifelse(produto_base == 'Farinha de trigo', preco*1.5,
                                                                               ifelse(produto_base == 'Tomate kg', preco*9,
                                                                                      ifelse(produto_base == 'Café', preco*0.6,
                                                                                             ifelse(produto_base == 'Banana kg', preco_duzia*7.5, preco*6))))))))))) -> Base_GO_reg

# Armazenando a base

SOAR::Store(Base_GO_reg, lib = R_Tese)
...
Base_GO_reg$mes <- as.character(Base_GO_reg$mes)
...
Base_GO_reg %>%
  dplyr::filter(teste == 'FALSE',
                ...
                mes != '202303',
                !(is.na(preco_cesta)),
                preco_cesta != 0) -> Base_GO_reg

Base_GO_reg$produto_base <- plyr::mapvalues(Base_GO_reg$produto_base,
...
DIEESE %>%
  dplyr::filter(UF == 'GO') %>%
  dplyr::select(-UF) %>%
  as.data.frame() -> DIEESE_GO

DIEESE_GO %>%
  dplyr::filter(produto_base == 'Total') %>%
  dplyr::select(-produto_base) -> DIEESE_GO_total

Base_GO_reg %>%
  dplyr::left_join(DIEESE_GO, by = c('produto_base', 'mes')) -> Cesta_BasicaIfood_GO

cv<-function(x){coef<-sd(x)/mean(x)*100 
return(coef)}

Cesta_BasicaIfood_GO %>%
...
# Cesta_BasicaIfood_GO %>%
#   dplyr::group_by(produto_base) %>%
#   dplyr::mutate(teste2 = ifelse(produto_base == 'Carne' & preco_cesta >= median(preco_cesta)*0.8 & preco_cesta <= median(preco_cesta)*10, 'FALSE',
#                                 ifelse(produto_base == 'Banana' & preco_cesta >= median(preco_cesta)*2.6 & preco_cesta <= median(preco_cesta)*7.6, 'FALSE',
#                                        ifelse(produto_base == 'Batata' & preco_cesta >= median(preco_cesta)*0.2 & preco_cesta <= median(preco_cesta)*1.5, 'FALSE',
#                                               ifelse(produto_base == 'Tomate' & preco_cesta >= median(preco_cesta)*0.5 & preco_cesta <= median(preco_cesta)*4.3, 'FALSE',
#                                                      ifelse(produto_base == 'Pão' & preco_cesta >= median(preco_cesta)*1 & preco_cesta <= median(preco_cesta)*21, 'FALSE',
...
#   dplyr::left_join(DIEESE_GO_total, by = 'mes') %>%
#   dplyr::mutate(diferenca = (preco_calculado - preco_oficial)/preco_oficial) -> Cesta_Final_GO
...
Cesta_BasicaIfood_GO %>%
  ...
  dplyr:: select(mes, preco_calculado, variancia, desvio_padrao, coef_var,
                 lim_inf, lim_sup, preco_oficial, diferenca)-> Cesta_Final_GO

openxlsx::write.xlsx(Cesta_Final_GO, file='Cesta_BasicaIfood_GO.xlsx')
