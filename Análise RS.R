...
'C:\\Users\\marcu\\Documents\\Tese\\Script\\Bases Ifood\\RS' -> diretorio
...
Base_RS2 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                localidade = dia, dia = hora, hora = X) -> Base_RS2

Base_RS3 <- data.frame()

for(i_arq in 212:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RS3, res) -> Base_RS3
}
...
Base_RS4 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_RS4
...
Base_RS6 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_RS6

Base_RS7 <- data.frame()

for(i_arq in 252:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RS7, res) -> Base_RS7
}
...
Base_RS8 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_RS8

Base_RS9 <- data.frame()

for(i_arq in 304:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RS9, res) -> Base_RS9
}

Base_RS10 <- data.frame()

for(i_arq in 316:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RS10, res) -> Base_RS10
}
...
Base_RS10 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_RS10

Base_RS11 <- data.frame()

for(i_arq in 317:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RS11, res) -> Base_RS11
}

Base_RS12 <- data.frame()

for(i_arq in 348:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RS12, res) -> Base_RS12
}
...
Base_RS12 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_RS12

Base_RS13 <- data.frame()

for(i_arq in 349:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RS13, res) -> Base_RS13
}

Base_RS14 <- data.frame()

for(i_arq in 353:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RS14, res) -> Base_RS14
}
...
Base_RS14 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_RS14

Base_RS15 <- data.frame()

for(i_arq in 354:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RS15, res) -> Base_RS15
}

Base_RS16 <- data.frame()

for(i_arq in 356:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RS16, res) -> Base_RS16
}
...
Base_RS16 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_RS16

Base_RS17 <- data.frame()

for(i_arq in 357:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RS17, res) -> Base_RS17
}
...
rbind(Base_RS, Base_RS2) -> Base_total_RS
rbind(Base_total_RS, Base_RS3) -> Base_total_RS
rbind(Base_total_RS, Base_RS4) -> Base_total_RS
rbind(Base_total_RS, Base_RS5) -> Base_total_RS
rbind(Base_total_RS, Base_RS6) -> Base_total_RS
rbind(Base_total_RS, Base_RS7) -> Base_total_RS
rbind(Base_total_RS, Base_RS8) -> Base_total_RS
rbind(Base_total_RS, Base_RS9) -> Base_total_RS
rbind(Base_total_RS, Base_RS10) -> Base_total_RS
rbind(Base_total_RS, Base_RS11) -> Base_total_RS
rbind(Base_total_RS, Base_RS12) -> Base_total_RS
rbind(Base_total_RS, Base_RS13) -> Base_total_RS
rbind(Base_total_RS, Base_RS14) -> Base_total_RS
rbind(Base_total_RS, Base_RS15) -> Base_total_RS
rbind(Base_total_RS, Base_RS16) -> Base_total_RS
rbind(Base_total_RS, Base_RS17) -> Base_total_RS

# Ajustando a base sem dados relevantes

Base_total_RS %>%
  dplyr::filter(!(hora == ""),
                !(dia == "Avenida Julio de Castilhos, 596, Centro, Porto Alegre - RS, Brasil")) -> Base_total_RS_ajust

# Ajustando os dias do ano para um único padrão: DD/MM/AAAA

unique(Base_total_RS_ajust$dia) -> f
...
Base_total_RS_ajust$dia <- plyr::mapvalues(Base_total_RS_ajust$dia,
                                          from = f,
                                          to = t)
...
SOAR::Store(Base_total_RS_ajust, lib = R_Tese)
...
Base_total_RS_ajust$teste <- apply(sapply(array , grepl, Base_total_RS_ajust$nome_produto), 1, any)
...
Base_total_RS_ajust$preco_produto <- as.numeric(Base_total_RS_ajust$preco_produto)
                                                                                      ...
                                                                                      ifelse(produto_base == 'Café', preco*0.6,
                                                                                             ifelse(produto_base == 'Batata kg', preco*6,
                                                                                                    ifelse(produto_base == 'Pão francês', preco*6,
                                                                                                           ifelse(produto_base == 'Banana kg', preco_duzia*7.5, preco*6.6))))))))))))) -> Base_RS_reg

# Armazenando a base

SOAR::Store(Base_RS_reg, lib = R_Tese)
...
Base_RS_reg$mes <- as.character(Base_RS_reg$mes)
...
Base_RS_reg %>%
  dplyr::filter(teste == 'FALSE',
                ...
                mes != '202303',
                !(is.na(preco_cesta)),
                preco_cesta != 0) -> Base_RS_reg

Base_RS_reg$produto_base <- plyr::mapvalues(Base_RS_reg$produto_base,
...
DIEESE %>%
  dplyr::filter(UF == 'RS') %>%
  dplyr::select(-UF) %>%
  as.data.frame() -> DIEESE_RS

DIEESE_RS %>%
  dplyr::filter(produto_base == 'Total') %>%
  dplyr::select(-produto_base) -> DIEESE_RS_total

Base_RS_reg %>%
  dplyr::left_join(DIEESE_RS, by = c('produto_base', 'mes')) -> Cesta_BasicaIfood_RS

cv<-function(x){coef<-sd(x)/mean(x)*100 
return(coef)}

Cesta_BasicaIfood_RS %>%
...
# Cesta_BasicaIfood_RS %>%
#   dplyr::group_by(produto_base) %>%
#   dplyr::mutate(teste2 = ifelse(produto_base == 'Carne' & preco_cesta >= median(preco_cesta)*1.3 & preco_cesta <= median(preco_cesta)*2.9, 'FALSE',
#                                 ifelse(produto_base == 'Banana' & preco_cesta >= median(preco_cesta)*0.8 & preco_cesta <= median(preco_cesta)*8.4, 'FALSE',
#                                        ifelse(produto_base == 'Batata' & preco_cesta >= median(preco_cesta)*2 & preco_cesta <= median(preco_cesta)*4.9, 'FALSE',
#                                               ifelse(produto_base == 'Tomate' & preco_cesta >= median(preco_cesta)*0.9 & preco_cesta <= median(preco_cesta)*3.4, 'FALSE',
#                                                      ifelse(produto_base == 'Pão' & preco_cesta >= median(preco_cesta)*1.4 & preco_cesta <= median(preco_cesta)*5.9, 'FALSE',
...
#   dplyr::left_join(DIEESE_RS_total, by = 'mes') %>%
#   dplyr::mutate(diferenca = (preco_calculado - preco_oficial)/preco_oficial) -> Cesta_Final_RS
...
Cesta_BasicaIfood_RS %>%
  ...
  dplyr:: select(mes, preco_calculado, variancia, desvio_padrao, coef_var,
                 lim_inf, lim_sup, preco_oficial, diferenca)-> Cesta_Final_RS

openxlsx::write.xlsx(Cesta_Final_RS, file='Cesta_BasicaIfood_RS.xlsx')
