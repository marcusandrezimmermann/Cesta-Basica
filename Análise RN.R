...
'C:\\Users\\marcu\\Documents\\Tese\\Script\\Bases Ifood\\RN' -> diretorio
...
Base_RN2 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                localidade = dia, dia = hora, hora = X) -> Base_RN2

Base_RN3 <- data.frame()

for(i_arq in 218:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RN3, res) -> Base_RN3
}
...
Base_RN4 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_RN4
...
Base_RN6 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_RN6

Base_RN7 <- data.frame()

for(i_arq in 311:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RN7, res) -> Base_RN7
}
...
Base_RN8 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_RN8

Base_RN9 <- data.frame()

for(i_arq in 328:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RN9, res) -> Base_RN9
}

Base_RN10 <- data.frame()

for(i_arq in 329:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RN10, res) -> Base_RN10
}
...
Base_RN10 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_RN10


Base_RN11 <- data.frame()

for(i_arq in 330:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RN11, res) -> Base_RN11
}

Base_RN12 <- data.frame()

for(i_arq in 351:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RN12, res) -> Base_RN12
}
...
Base_RN12 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_RN12

Base_RN13 <- data.frame()

for(i_arq in 352:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RN13, res) -> Base_RN13
}

Base_RN14 <- data.frame()

for(i_arq in 353:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RN14, res) -> Base_RN14
}
...
Base_RN14 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_RN14

Base_RN15 <- data.frame()

for(i_arq in 354:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_RN15, res) -> Base_RN15
}
...
rbind(Base_RN, Base_RN2) -> Base_total_RN
rbind(Base_total_RN, Base_RN3) -> Base_total_RN
rbind(Base_total_RN, Base_RN4) -> Base_total_RN
rbind(Base_total_RN, Base_RN5) -> Base_total_RN
rbind(Base_total_RN, Base_RN6) -> Base_total_RN
rbind(Base_total_RN, Base_RN7) -> Base_total_RN
rbind(Base_total_RN, Base_RN8) -> Base_total_RN
rbind(Base_total_RN, Base_RN9) -> Base_total_RN
rbind(Base_total_RN, Base_RN10) -> Base_total_RN
rbind(Base_total_RN, Base_RN11) -> Base_total_RN
rbind(Base_total_RN, Base_RN12) -> Base_total_RN
rbind(Base_total_RN, Base_RN13) -> Base_total_RN
rbind(Base_total_RN, Base_RN14) -> Base_total_RN
rbind(Base_total_RN, Base_RN15) -> Base_total_RN

# Ajustando a base sem dados relevantes

Base_total_RN %>%
  dplyr::filter(!(hora == "")) -> Base_total_RN_ajust

# Ajustando os dias do ano para um único padrão: DD/MM/AAAA

unique(Base_total_RN_ajust$dia) -> f
...
Base_total_RN_ajust$dia <- plyr::mapvalues(Base_total_RN_ajust$dia,
                                          from = f,
                                          to = t)
...
SOAR::Store(Base_total_RN_ajust, lib = R_Tese)
...
Base_total_RN_ajust$teste <- apply(sapply(array , grepl, Base_total_RN_ajust$nome_produto), 1, any)
...
Base_total_RN_ajust$preco_produto <- as.numeric(Base_total_RN_ajust$preco_produto)
                                                                        ...
                                                                        ifelse(produto_base == 'Farinha de mandioca', preco*3,
                                                                               ifelse(produto_base == 'Tomate kg', preco*12,
                                                                                      ifelse(produto_base == 'Café', preco*0.3,
                                                                                             ifelse(produto_base == 'Banana kg', preco_duzia*7.5, preco*4.5))))))))))) -> Base_RN_reg

# Armazenando a base

SOAR::Store(Base_RN_reg, lib = R_Tese)
...
Base_RN_reg$mes <- as.character(Base_RN_reg$mes)
...
Base_RN_reg %>%
  dplyr::filter(teste == 'FALSE',
                ...
                mes != '202303',
                !(is.na(preco_cesta)),
                preco_cesta != 0) -> Base_RN_reg

Base_RN_reg$produto_base <- plyr::mapvalues(Base_RN_reg$produto_base,
...
DIEESE %>%
  dplyr::filter(UF == 'RN') %>%
  dplyr::select(-UF) %>%
  as.data.frame() -> DIEESE_RN

DIEESE_RN %>%
  dplyr::filter(produto_base == 'Total') %>%
  dplyr::select(-produto_base) -> DIEESE_RN_total

Base_RN_reg %>%
  dplyr::left_join(DIEESE_RN, by = c('produto_base', 'mes')) -> Cesta_BasicaIfood_RN

cv<-function(x){coef<-sd(x)/mean(x)*100 
return(coef)}

Cesta_BasicaIfood_RN %>%
...
# Cesta_BasicaIfood_RN %>%
#   dplyr::group_by(produto_base) %>%
#   dplyr::mutate(teste2 = ifelse(produto_base == 'Carne' & preco_cesta >= median(preco_cesta)*1.2 & preco_cesta <= median(preco_cesta)*2.5, 'FALSE',
#                                 ifelse(produto_base == 'Banana' & preco_cesta >= median(preco_cesta)*3.4 & preco_cesta <= median(preco_cesta)*9.3, 'FALSE',
#                                        ifelse(produto_base == 'Açúcar' & preco_cesta >= median(preco_cesta)*0.6 & preco_cesta <= median(preco_cesta)*1.3, 'FALSE',
#                                               ifelse(produto_base == 'Tomate' & preco_cesta >= median(preco_cesta)*0.2 & preco_cesta <= median(preco_cesta)*2.7, 'FALSE',
#                                                      ifelse(produto_base == 'Pão' & preco_cesta >= median(preco_cesta)*0.7 & preco_cesta <= median(preco_cesta)*2.2, 'FALSE',
#                                                             ifelse(produto_base %in% c('Arroz', 'Feijão', 'Farinha', 'Café', 'Óleo', 'Manteiga', 'Leite') & preco_cesta >= median(preco_cesta)*0.5 & preco_cesta <= median(preco_cesta)*2.5, 'FALSE', 'TRUE'))))))) %>%
#   dplyr::ungroup() %>%
#   dplyr::filter(teste2 == 'FALSE') %>%
#   dplyr::group_by(produto_base, mes, preco_oficial) %>%
...
#   dplyr::left_join(DIEESE_RN_total, by = 'mes') %>%
#   dplyr::mutate(diferenca = (preco_calculado - preco_oficial)/preco_oficial) -> Cesta_Final_RN
...
Cesta_BasicaIfood_RN %>%
  ...
  dplyr:: select(mes, preco_calculado, variancia, desvio_padrao, coef_var,
                 lim_inf, lim_sup, preco_oficial, diferenca) -> Cesta_Final_RN

openxlsx::write.xlsx(Cesta_Final_RN, file='Cesta_BasicaIfood_RN.xlsx')

# tmp %>%
#   dplyr::filter(mes == '202402') %>%
#   dplyr::select(produto_base) %>%
#   unique()