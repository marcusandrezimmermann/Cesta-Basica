...
'C:\\Users\\marcu\\Documents\\Tese\\Script\\Bases Ifood\\PA' -> diretorio
...
for(i_arq in 1:length(arquivos)) {
  ...
  rbind(Base_PA, res) -> Base_PA
}

Base_PA2 <- data.frame()

for(i_arq in 238:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_PA2, res) -> Base_PA2
}
...
Base_PA2 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                localidade = dia, dia = hora, hora = X) -> Base_PA2

Base_PA3 <- data.frame()

for(i_arq in 239:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_PA3, res) -> Base_PA3
}
...
rbind(Base_PA, Base_PA2) -> Base_total_PA
rbind(Base_total_PA, Base_PA3) -> Base_total_PA
rbind(Base_total_PA, Base_PA4) -> Base_total_PA

# Ajustando a base sem dados relevantes

Base_total_PA %>%
  unique() -> Base_total_PA

Base_total_PA %>%
  dplyr::filter(!(hora == "")) -> Base_total_PA_ajust

# Ajustando os dias do ano para um único padrão: DD/MM/AAAA

unique(Base_total_PA_ajust$dia) -> f
...
Base_total_PA_ajust$dia <- plyr::mapvalues(Base_total_PA_ajust$dia,
                                          from = f,
                                          to = t)
...
SOAR::Store(Base_total_PA_ajust, lib = R_Tese)
...
Base_total_PA_ajust$teste <- apply(sapply(array , grepl, Base_total_PA_ajust$nome_produto), 1, any)
...
Base_total_PA_ajust$preco_produto <- as.numeric(Base_total_PA_ajust$preco_produto)
                                                                        ...
                                                                        ifelse(produto_base == 'Farinha de mandioca', preco*3,
                                                                               ifelse(produto_base == 'Tomate kg', preco*12,
                                                                                      ifelse(produto_base == 'Café', preco*0.3,
                                                                                             ifelse(produto_base == 'Banana kg', preco_duzia*7.5, preco*4.5))))))))))) -> Base_PA_reg

# Armazenando a base

SOAR::Store(Base_PA_reg, lib = R_Tese)
...
Base_PA_reg$mes <- as.character(Base_PA_reg$mes)
...
Base_PA_reg %>%
  dplyr::filter(teste == 'FALSE',
                ...
                mes != '202303',
                !(is.na(preco_cesta)),
                preco_cesta != 0) -> Base_PA_reg

Base_PA_reg$produto_base <- plyr::mapvalues(Base_PA_reg$produto_base,
...
DIEESE %>%
  dplyr::filter(UF == 'PA') %>%
  dplyr::select(-UF) %>%
  as.data.frame() -> DIEESE_PA

DIEESE_PA %>%
  dplyr::filter(produto_base == 'Total') %>%
  dplyr::select(-produto_base) -> DIEESE_PA_total

Base_PA_reg %>%
  dplyr::left_join(DIEESE_PA, by = c('produto_base', 'mes')) -> Cesta_BasicaIfood_PA

cv<-function(x){coef<-sd(x)/mean(x)*100 
return(coef)}

Cesta_BasicaIfood_PA %>%
...
# Cesta_BasicaIfood_PA %>%
#   dplyr::group_by(produto_base) %>%
#   dplyr::mutate(teste2 = ifelse(produto_base == 'Carne' & preco_cesta >= median(preco_cesta)*1.8 & preco_cesta <= median(preco_cesta)*6, 'FALSE',
#                                 ifelse(produto_base == 'Banana' & preco_cesta >= median(preco_cesta)*3 & preco_cesta <= median(preco_cesta)*8.5, 'FALSE',
#                                        ifelse(produto_base == 'Açúcar' & preco_cesta >= median(preco_cesta)*0.5 & preco_cesta <= median(preco_cesta)*1.5, 'FALSE',
#                                               ifelse(produto_base == 'Tomate' & preco_cesta >= median(preco_cesta)*1.8 & preco_cesta <= median(preco_cesta)*5.6, 'FALSE',
#                                                      ifelse(produto_base == 'Pão' & preco_cesta >= median(preco_cesta)*0.1 & preco_cesta <= median(preco_cesta)*1.2, 'FALSE',
...
#   dplyr::left_join(DIEESE_PA_total, by = 'mes') %>%
#   dplyr::mutate(diferenca = (preco_calculado - preco_oficial)/preco_oficial) -> Cesta_Final_PA
...
Cesta_BasicaIfood_PA %>%
  ...
  dplyr:: select(mes, preco_calculado, variancia, desvio_padrao, coef_var,
                 lim_inf, lim_sup, preco_oficial, diferenca)-> Cesta_Final_PA

openxlsx::write.xlsx(Cesta_Final_PA, file='Cesta_BasicaIfood_PA.xlsx')
