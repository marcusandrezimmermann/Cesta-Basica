...
'C:\\Users\\marcu\\Documents\\Tese\\Script\\Bases Ifood\\SE' -> diretorio
...
Base_SE2 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                localidade = dia, dia = hora, hora = X) -> Base_SE2

Base_SE3 <- data.frame()

for(i_arq in 327:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_SE3, res) -> Base_SE3
}
...
Base_SE4 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_SE4
...
rbind(Base_SE, Base_SE2) -> Base_total_SE
rbind(Base_total_SE, Base_SE3) -> Base_total_SE
rbind(Base_total_SE, Base_SE4) -> Base_total_SE
rbind(Base_total_SE, Base_SE5) -> Base_total_SE

# Ajustando a base sem dados relevantes

Base_total_SE %>%
  dplyr::filter(!(hora == "")) -> Base_total_SE_ajust

# Ajustando os dias do ano para um único padrão: DD/MM/AAAA

unique(Base_total_SE_ajust$dia) -> f
...
Base_total_SE_ajust$dia <- plyr::mapvalues(Base_total_SE_ajust$dia,
                                          from = f,
                                          to = t)
...
SOAR::Store(Base_total_SE_ajust, lib = R_Tese)
...
Base_total_SE_ajust$teste <- apply(sapply(array , grepl, Base_total_SE_ajust$nome_produto), 1, any)
...
Base_total_SE_ajust$preco_produto <- as.numeric(Base_total_SE_ajust$preco_produto)
                                                                        ...
                                                                        ifelse(produto_base == 'Farinha de mandioca', preco*3,
                                                                               ifelse(produto_base == 'Tomate kg', preco*12,
                                                                                      ifelse(produto_base == 'Café', preco*0.3,
                                                                                             ifelse(produto_base == 'Banana kg', preco_duzia*7.5, preco*4.5))))))))))) -> Base_SE_reg

# Armazenando a base

SOAR::Store(Base_SE_reg, lib = R_Tese)
...
Base_SE_reg$mes <- as.character(Base_SE_reg$mes)
...
Base_SE_reg %>%
  dplyr::filter(teste == 'FALSE',
                ...
                mes != '202303',
                !(is.na(preco_cesta)),
                preco_cesta != 0) -> Base_SE_reg

Base_SE_reg$produto_base <- plyr::mapvalues(Base_SE_reg$produto_base,
...
DIEESE %>%
  dplyr::filter(UF == 'SE') %>%
  dplyr::select(-UF) %>%
  as.data.frame() -> DIEESE_SE

DIEESE_SE %>%
  dplyr::filter(produto_base == 'Total') %>%
  dplyr::select(-produto_base) -> DIEESE_SE_total

Base_SE_reg %>%
  dplyr::left_join(DIEESE_SE, by = c('produto_base', 'mes')) -> Cesta_BasicaIfood_SE

cv<-function(x){coef<-sd(x)/mean(x)*100 
return(coef)}

Cesta_BasicaIfood_SE %>%
...
# Cesta_BasicaIfood_SE %>%
#   dplyr::group_by(produto_base) %>%
#   dplyr::mutate(teste2 = ifelse(produto_base == 'Carne' & preco_cesta >= median(preco_cesta)*1.3 & preco_cesta <= median(preco_cesta)*3, 'FALSE',
#                                 ifelse(produto_base == 'Banana' & preco_cesta >= median(preco_cesta)*2.3 & preco_cesta <= median(preco_cesta)*6.4, 'FALSE',
#                                        ifelse(produto_base == 'Açúcar' & preco_cesta >= median(preco_cesta)*0.4 & preco_cesta <= median(preco_cesta)*1.1, 'FALSE',
#                                               ifelse(produto_base == 'Tomate' & preco_cesta >= median(preco_cesta)*0.5 & preco_cesta <= median(preco_cesta)*4.6, 'FALSE',
#                                                      ifelse(produto_base == 'Pão' & preco_cesta >= median(preco_cesta)*1.3 & preco_cesta <= median(preco_cesta)*7.8, 'FALSE',
...
#   dplyr::left_join(DIEESE_SE_total, by = 'mes') %>%
#   dplyr::mutate(diferenca = (preco_calculado - preco_oficial)/preco_oficial) -> Cesta_Final_SE
...
Cesta_BasicaIfood_SE %>%
  ...
  dplyr:: select(mes, preco_calculado, variancia, desvio_padrao, coef_var,
                 lim_inf, lim_sup, preco_oficial, diferenca) -> Cesta_Final_SE

openxlsx::write.xlsx(Cesta_Final_SE, file='Cesta_BasicaIfood_SE.xlsx')
