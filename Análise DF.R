...
'C:\\Users\\marcu\\Documents\\Tese\\Script\\Bases Ifood\\DF' -> diretorio
...
Base_DF2 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                localidade = dia, dia = hora, hora = X) -> Base_DF2

Base_DF3 <- data.frame()

for(i_arq in 309:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_DF3, res) -> Base_DF3
}
...
Base_DF4 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_DF4
...
rbind(Base_DF, Base_DF2) -> Base_total_DF
rbind(Base_total_DF, Base_DF3) -> Base_total_DF
rbind(Base_total_DF, Base_DF4) -> Base_total_DF
rbind(Base_total_DF, Base_DF5) -> Base_total_DF

# Ajustando a base sem dados relevantes

Base_total_DF %>%
  dplyr::filter(!(hora == "")) -> Base_total_DF_ajust

# Ajustando os dias do ano para um único padrão: DD/MM/AAAA

unique(Base_total_DF_ajust$dia) -> f
...
Base_total_DF_ajust$dia <- plyr::mapvalues(Base_total_DF_ajust$dia,
                                          from = f,
                                          to = t)
...
SOAR::Store(Base_total_DF_ajust, lib = R_Tese)
...
Base_total_DF_ajust$teste <- apply(sapply(array , grepl, Base_total_DF_ajust$nome_produto), 1, any)
...
Base_total_DF_ajust$preco_produto <- as.numeric(Base_total_DF_ajust$preco_produto)
                                                                        ...
                                                                        ifelse(produto_base == 'Farinha de trigo', preco*1.5,
                                                                               ifelse(produto_base == 'Tomate kg', preco*9,
                                                                                      ifelse(produto_base == 'Café', preco*0.6,
                                                                                             ifelse(produto_base == 'Banana kg', preco_duzia*7.5, preco*6))))))))))) -> Base_DF_reg

# Armazenando a base

SOAR::Store(Base_DF_reg, lib = R_Tese)
...
Base_DF_reg$mes <- as.character(Base_DF_reg$mes)
...
Base_DF_reg %>%
  dplyr::filter(mes != '202303',
                ...
                !(is.na(preco_cesta)),
                preco_cesta != 0) -> Base_DF_reg

Base_DF_reg$produto_base <- plyr::mapvalues(Base_DF_reg$produto_base,
...
DIEESE %>%
  dplyr::filter(UF == 'DF') %>%
  dplyr::select(-UF) %>%
  as.data.frame() -> DIEESE_DF

DIEESE_DF %>%
  dplyr::filter(produto_base == 'Total') %>%
  dplyr::select(-produto_base) -> DIEESE_DF_total

Base_DF_reg %>%
  dplyr::left_join(DIEESE_DF, by = c('produto_base', 'mes')) -> Cesta_BasicaIfood_DF

cv<-function(x){coef<-sd(x)/mean(x)*100
return(coef)}

Cesta_BasicaIfood_DF %>%
...
# Cesta_BasicaIfood_DF %>%
#   dplyr::group_by(produto_base) %>%
#   dplyr::mutate(teste2 = ifelse(produto_base == 'Carne' & preco_cesta >= median(preco_cesta)*1.2 & preco_cesta <= median(preco_cesta)*3.5, 'FALSE',
#                                 ifelse(produto_base == 'Banana' & preco_cesta >= median(preco_cesta)*0.6 & preco_cesta <= median(preco_cesta)*2.2, 'FALSE',
#                                        ifelse(produto_base == 'Batata' & preco_cesta >= median(preco_cesta)*0.6 & preco_cesta <= median(preco_cesta)*3.2, 'FALSE',
#                                               ifelse(produto_base == 'Tomate' & preco_cesta >= median(preco_cesta)*0.2 & preco_cesta <= median(preco_cesta)*2.8, 'FALSE',
#                                                      ifelse(produto_base == 'Pão' & preco_cesta >= median(preco_cesta)*1 & preco_cesta <= median(preco_cesta)*10, 'FALSE',
...
#   dplyr::left_join(DIEESE_DF_total, by = 'mes') %>%
#   dplyr::mutate(diferenca = (preco_calculado - preco_oficial)/preco_oficial) -> Cesta_Final_DF
...
Cesta_BasicaIfood_DF %>%
  ...
  dplyr:: select(mes, preco_calculado, variancia, desvio_padrao, coef_var,
                 lim_inf, lim_sup, preco_oficial, diferenca) -> Cesta_Final_DF

openxlsx::write.xlsx(Cesta_Final_DF, file='Cesta_BasicaIfood_DF.xlsx')
