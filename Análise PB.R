...
'C:\\Users\\marcu\\Documents\\Tese\\Script\\Bases Ifood\\PB' -> diretorio
...
Base_PB2 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                localidade = dia, dia = hora, hora = X) -> Base_PB2

Base_PB3 <- data.frame()

for(i_arq in 217:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_PB3, res) -> Base_PB3
}
...
Base_PB4 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_PB4
...
Base_PB6 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_PB6

Base_PB7 <- data.frame()

for(i_arq in 253:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_PB7, res) -> Base_PB7
}
...
Base_PB8 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_PB8

Base_PB9 <- data.frame()

for(i_arq in 307:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_PB9, res) -> Base_PB9
}

Base_PB10 <- data.frame()

for(i_arq in 314:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_PB10, res) -> Base_PB10
}
...
Base_PB10 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_PB10


Base_PB11 <- data.frame()

for(i_arq in 315:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_PB11, res) -> Base_PB11
}

Base_PB12 <- data.frame()

for(i_arq in 325:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_PB12, res) -> Base_PB12
}
...
Base_PB12 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_PB12

Base_PB13 <- data.frame()

for(i_arq in 326:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_PB13, res) -> Base_PB13
}

Base_PB14 <- data.frame()

for(i_arq in 340:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_PB14, res) -> Base_PB14
}
...
Base_PB14 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_PB14

Base_PB15 <- data.frame()

for(i_arq in 341:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_PB15, res) -> Base_PB15
}

Base_PB16 <- data.frame()

for(i_arq in 344:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_PB16, res) -> Base_PB16
}
...
Base_PB16 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_PB16

Base_PB17 <- data.frame()

for(i_arq in 345:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_PB17, res) -> Base_PB17
}
...
rbind(Base_PB, Base_PB2) -> Base_total_PB
rbind(Base_total_PB, Base_PB3) -> Base_total_PB
rbind(Base_total_PB, Base_PB4) -> Base_total_PB
rbind(Base_total_PB, Base_PB5) -> Base_total_PB
rbind(Base_total_PB, Base_PB6) -> Base_total_PB
rbind(Base_total_PB, Base_PB7) -> Base_total_PB
rbind(Base_total_PB, Base_PB8) -> Base_total_PB
rbind(Base_total_PB, Base_PB9) -> Base_total_PB
rbind(Base_total_PB, Base_PB10) -> Base_total_PB
rbind(Base_total_PB, Base_PB11) -> Base_total_PB
rbind(Base_total_PB, Base_PB12) -> Base_total_PB
rbind(Base_total_PB, Base_PB13) -> Base_total_PB
rbind(Base_total_PB, Base_PB14) -> Base_total_PB
rbind(Base_total_PB, Base_PB15) -> Base_total_PB
rbind(Base_total_PB, Base_PB16) -> Base_total_PB
rbind(Base_total_PB, Base_PB17) -> Base_total_PB

# Ajustando a base sem dados relevantes

Base_total_PB %>%
  dplyr::filter(!(hora == "")) -> Base_total_PB_ajust

# Ajustando os dias do ano para um único padrão: DD/MM/AAAA

unique(Base_total_PB_ajust$dia) -> f
...
Base_total_PB_ajust$dia <- plyr::mapvalues(Base_total_PB_ajust$dia,
                                          from = f,
                                          to = t)
...
SOAR::Store(Base_total_PB_ajust, lib = R_Tese)
...
Base_total_PB_ajust$teste <- apply(sapply(array , grepl, Base_total_PB_ajust$nome_produto), 1, any)
...
Base_total_PB_ajust$preco_produto <- as.numeric(Base_total_PB_ajust$preco_produto)
                                                                        ...
                                                                        ifelse(produto_base == 'Farinha de mandioca', preco*3,
                                                                               ifelse(produto_base == 'Tomate kg', preco*12,
                                                                                      ifelse(produto_base == 'Café', preco*0.3,
                                                                                             ifelse(produto_base == 'Banana kg', preco_duzia*7.5, preco*4.5))))))))))) -> Base_PB_reg

# Armazenando a base

SOAR::Store(Base_PB_reg, lib = R_Tese)
...
Base_PB_reg$mes <- as.character(Base_PB_reg$mes)
...
Base_PB_reg %>%
  dplyr::filter(teste == 'FALSE',
                ...
                mes != '202303',
                !(is.na(preco_cesta)),
                preco_cesta != 0) -> Base_PB_reg

Base_PB_reg$produto_base <- plyr::mapvalues(Base_PB_reg$produto_base,
...
DIEESE %>%
  dplyr::filter(UF == 'PB') %>%
  dplyr::select(-UF) %>%
  as.data.frame() -> DIEESE_PB

DIEESE_PB %>%
  dplyr::filter(produto_base == 'Total') %>%
  dplyr::select(-produto_base) -> DIEESE_PB_total

Base_PB_reg %>%
  dplyr::left_join(DIEESE_PB, by = c('produto_base', 'mes')) -> Cesta_BasicaIfood_PB

cv<-function(x){coef<-sd(x)/mean(x)*100 
return(coef)}

Cesta_BasicaIfood_PB %>%
...
# Cesta_BasicaIfood_PB %>%
#   dplyr::group_by(produto_base) %>%
#   dplyr::mutate(teste2 = ifelse(produto_base == 'Carne' & preco_cesta >= median(preco_cesta)*0.7 & preco_cesta <= median(preco_cesta)*4, 'FALSE',
#                                 ifelse(produto_base == 'Banana' & preco_cesta >= median(preco_cesta)*1.6 & preco_cesta <= median(preco_cesta)*8.1, 'FALSE',
#                                        ifelse(produto_base == 'Açúcar' & preco_cesta >= median(preco_cesta)*0.4 & preco_cesta <= median(preco_cesta)*1.2, 'FALSE',
#                                               ifelse(produto_base == 'Tomate' & preco_cesta >= median(preco_cesta)*0.2 & preco_cesta <= median(preco_cesta)*2.4, 'FALSE',
#                                                      ifelse(produto_base == 'Pão' & preco_cesta >= median(preco_cesta)*2.9 & preco_cesta <= median(preco_cesta)*11, 'FALSE',
...
#   dplyr::left_join(DIEESE_PB_total, by = 'mes') %>%
#   dplyr::mutate(diferenca = (preco_calculado - preco_oficial)/preco_oficial) -> Cesta_Final_PB
...
Cesta_BasicaIfood_PB %>%
  ...
  dplyr:: select(mes, preco_calculado, variancia, desvio_padrao, coef_var,
                 lim_inf, lim_sup, preco_oficial, diferenca)-> Cesta_Final_PB

openxlsx::write.xlsx(Cesta_Final_PB, file='Cesta_BasicaIfood_PB.xlsx')
