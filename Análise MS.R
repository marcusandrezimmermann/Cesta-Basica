...
'C:\\Users\\marcu\\Documents\\Tese\\Script\\Bases Ifood\\MS' -> diretorio
...
for(i_arq in 1:length(arquivos)) {
  ...
  rbind(Base_MS, res) -> Base_MS
}

Base_MS2 <- data.frame()

for(i_arq in 278:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_MS2, res) -> Base_MS2
}
...
Base_MS2 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                localidade = dia, dia = hora, hora = X) -> Base_MS2

Base_MS3 <- data.frame()

for(i_arq in 279:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_MS3, res) -> Base_MS3
}
...
Base_MS4 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_MS4
...
Base_MS6 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_MS6

Base_MS7 <- data.frame()

for(i_arq in 349:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_MS7, res) -> Base_MS7
}
...
rbind(Base_MS, Base_MS2) -> Base_total_MS
rbind(Base_total_MS, Base_MS3) -> Base_total_MS
rbind(Base_total_MS, Base_MS4) -> Base_total_MS
rbind(Base_total_MS, Base_MS5) -> Base_total_MS
rbind(Base_total_MS, Base_MS6) -> Base_total_MS
rbind(Base_total_MS, Base_MS7) -> Base_total_MS

# Ajustando a base sem dados relevantes

Base_total_MS %>%
  dplyr::filter(!(hora == "")) -> Base_total_MS_ajust

# Ajustando os dias do ano para um único padrão: DD/MM/AAAA

unique(Base_total_MS_ajust$dia) -> f
...
Base_total_MS_ajust$dia <- plyr::mapvalues(Base_total_MS_ajust$dia,
                                          from = f,
                                          to = t)
...
SOAR::Store(Base_total_MS_ajust, lib = R_Tese)
...
Base_total_MS_ajust$teste <- apply(sapply(array , grepl, Base_total_MS_ajust$nome_produto), 1, any)
...
Base_total_MS_ajust$preco_produto <- as.numeric(Base_total_MS_ajust$preco_produto)
                                                                                      ...
                                                                                      ifelse(produto_base == 'Café', preco*0.6,
                                                                                             ifelse(produto_base == 'Batata kg', preco*6,
                                                                                                    ifelse(produto_base == 'Pão francês', preco*6,
                                                                                                           ifelse(produto_base == 'Banana kg', preco_duzia*7.5, preco*6.6))))))))))))) -> Base_MS_reg

# Armazenando a base

SOAR::Store(Base_MS_reg, lib = R_Tese)
...
Base_MS_reg$mes <- as.character(Base_MS_reg$mes)
...
Base_MS_reg %>%
  dplyr::filter(teste == 'FALSE',
                ...
                mes != '202303',
                !(is.na(preco_cesta)),
                preco_cesta != 0) -> Base_MS_reg

Base_MS_reg$produto_base <- plyr::mapvalues(Base_MS_reg$produto_base,
...
DIEESE %>%
  dplyr::filter(UF == 'MS') %>%
  dplyr::select(-UF) %>%
  as.data.frame() -> DIEESE_MS

DIEESE_MS %>%
  dplyr::filter(produto_base == 'Total') %>%
  dplyr::select(-produto_base) -> DIEESE_MS_total

Base_MS_reg %>%
  dplyr::left_join(DIEESE_MS, by = c('produto_base', 'mes')) -> Cesta_BasicaIfood_MS

cv<-function(x){coef<-sd(x)/mean(x)*100 
return(coef)}

Cesta_BasicaIfood_MS %>%
...
# Cesta_BasicaIfood_MS %>%
#   dplyr::group_by(produto_base) %>%
#   dplyr::mutate(teste2 = ifelse(produto_base == 'Carne' & preco_cesta >= median(preco_cesta)*1 & preco_cesta <= median(preco_cesta)*9.3, 'FALSE',
#                                 ifelse(produto_base == 'Banana' & preco_cesta >= median(preco_cesta)*0.8 & preco_cesta <= median(preco_cesta)*5.1, 'FALSE',
#                                        ifelse(produto_base == 'Batata' & preco_cesta >= median(preco_cesta)*0.5 & preco_cesta <= median(preco_cesta)*2.2, 'FALSE',
#                                               ifelse(produto_base == 'Tomate' & preco_cesta >= median(preco_cesta)*0.8 & preco_cesta <= median(preco_cesta)*2.5, 'FALSE',
#                                                      ifelse(produto_base == 'Pão' & preco_cesta >= median(preco_cesta)*0.2 & preco_cesta <= median(preco_cesta)*2.4, 'FALSE',
#                                                             ifelse(produto_base %in% c('Açúcar', 'Arroz', 'Feijão', 'Farinha', 'Café', 'Óleo', 'Manteiga', 'Leite') & preco_cesta >= median(preco_cesta)*0.5 & preco_cesta <= median(preco_cesta)*10, 'FALSE', 'TRUE'))))))) %>%
#   dplyr::ungroup() %>%
#   dplyr::filter(teste2 == 'FALSE') %>%
...
#   dplyr::left_join(DIEESE_MS_total, by = 'mes') %>%
#   dplyr::mutate(diferenca = (preco_calculado - preco_oficial)/preco_oficial) -> Cesta_Final_MS
...
Cesta_BasicaIfood_MS %>%
  ...
  dplyr:: select(mes, preco_calculado, variancia, desvio_padrao, coef_var,
                 lim_inf, lim_sup, preco_oficial, diferenca)-> Cesta_Final_MS

openxlsx::write.xlsx(Cesta_Final_MS, file='Cesta_BasicaIfood_MS.xlsx')

# tmp %>%
#   dplyr::filter(mes == '202304') %>%
#   dplyr::select(produto_base) %>%
#   unique()
