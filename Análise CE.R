...
'C:\\Users\\marcu\\Documents\\Tese\\Script\\Bases Ifood\\CE' -> diretorio
...
for(i_arq in 1:length(arquivos)) {
  ...
  rbind(Base_CE, res) -> Base_CE
}

Base_CE2 <- data.frame()

for(i_arq in 215:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_CE2, res) -> Base_CE2
}
...
Base_CE2 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                localidade = dia, dia = hora, hora = X) -> Base_CE2

Base_CE3 <- data.frame()

for(i_arq in 217:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_CE3, res) -> Base_CE3
}
...
Base_CE4 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_CE4
...
Base_CE6 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_CE6

Base_CE7 <- data.frame()

for(i_arq in 306:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_CE7, res) -> Base_CE7
}
...
Base_CE8 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_CE8

Base_CE9 <- data.frame()

for(i_arq in 309:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_CE9, res) -> Base_CE9
}

Base_CE10 <- data.frame()

for(i_arq in 311:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_CE10, res) -> Base_CE10
}
...
Base_CE10 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_CE10


Base_CE11 <- data.frame()

for(i_arq in 314:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_CE11, res) -> Base_CE11
}

Base_CE12 <- data.frame()

for(i_arq in 316:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_CE12, res) -> Base_CE12
}
...
Base_CE12 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_CE12

Base_CE13 <- data.frame()

for(i_arq in 317:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_CE13, res) -> Base_CE13
}

Base_CE14 <- data.frame()

for(i_arq in 319:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_CE14, res) -> Base_CE14
}
...
Base_CE14 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_CE14

Base_CE15 <- data.frame()

for(i_arq in 320:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_CE15, res) -> Base_CE15
}

Base_CE16 <- data.frame()

for(i_arq in 324:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_CE16, res) -> Base_CE16
}
...
Base_CE16 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_CE16

Base_CE17 <- data.frame()

for(i_arq in 326:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_CE17, res) -> Base_CE17
}
...
rbind(Base_CE, Base_CE2) -> Base_total_CE
rbind(Base_total_CE, Base_CE3) -> Base_total_CE
rbind(Base_total_CE, Base_CE4) -> Base_total_CE
rbind(Base_total_CE, Base_CE5) -> Base_total_CE
rbind(Base_total_CE, Base_CE6) -> Base_total_CE
rbind(Base_total_CE, Base_CE7) -> Base_total_CE
rbind(Base_total_CE, Base_CE8) -> Base_total_CE
rbind(Base_total_CE, Base_CE9) -> Base_total_CE
rbind(Base_total_CE, Base_CE10) -> Base_total_CE
rbind(Base_total_CE, Base_CE11) -> Base_total_CE
rbind(Base_total_CE, Base_CE12) -> Base_total_CE
rbind(Base_total_CE, Base_CE13) -> Base_total_CE
rbind(Base_total_CE, Base_CE14) -> Base_total_CE
rbind(Base_total_CE, Base_CE15) -> Base_total_CE
rbind(Base_total_CE, Base_CE16) -> Base_total_CE
rbind(Base_total_CE, Base_CE17) -> Base_total_CE

# Ajustando a base sem dados relevantes

Base_total_CE %>%
  dplyr::filter(!(hora == "")) -> Base_total_CE_ajust

# Ajustando os dias do ano para um único padrão: DD/MM/AAAA

unique(Base_total_CE_ajust$dia) -> f
...
Base_total_CE_ajust$dia <- plyr::mapvalues(Base_total_CE_ajust$dia,
                                          from = f,
                                          to = t)
...
SOAR::Store(Base_total_CE_ajust, lib = R_Tese)
...
Base_total_CE_ajust$teste <- apply(sapply(array , grepl, Base_total_CE_ajust$nome_produto), 1, any)
...
Base_total_CE_ajust$preco_produto <- as.numeric(Base_total_CE_ajust$preco_produto)
                                                                        ...
                                                                        ifelse(produto_base == 'Farinha de mandioca', preco*3,
                                                                               ifelse(produto_base == 'Tomate kg', preco*12,
                                                                                      ifelse(produto_base == 'Café', preco*0.3,
                                                                                             ifelse(produto_base == 'Banana kg', preco_duzia*7.5, preco*4.5))))))))))) -> Base_CE_reg

# Armazenando a base

SOAR::Store(Base_CE_reg, lib = R_Tese)
...
Base_CE_reg$mes <- as.character(Base_CE_reg$mes)
...
Base_CE_reg %>%
  dplyr::filter(mes != '202303',
                ...
                !(is.na(preco_cesta)),
                preco_cesta != 0) -> Base_CE_reg

Base_CE_reg$produto_base <- plyr::mapvalues(Base_CE_reg$produto_base,
                                            from = c("Coxão mole", "Coxão duro", "Patinho", "Feijão carioca", "Feijão preto", "Farinha de trigo", "Farinha de mandioca", "Arroz tipo 1", "Banana kg", "Açúcar refinado", "Batata Kg", "Leite integral 1L", "Óleo de soja", "Pão francês", "Tomate kg"),
                                            to = c("Carne", "Carne", "Carne", "Feijão", "Feijão", "Farinha", "Farinha", "Arroz", "Banana", "Açúcar", "Batata", "Leite", "Óleo", "Pão", "Tomate"))


DIEESE %>%
  dplyr::filter(UF == 'CE') %>%
  dplyr::select(-UF) %>%
  as.data.frame() -> DIEESE_CE

DIEESE_CE %>%
  dplyr::filter(produto_base == 'Total') %>%
  dplyr::select(-produto_base) -> DIEESE_CE_total

Base_CE_reg %>%
  dplyr::left_join(DIEESE_CE, by = c('produto_base', 'mes')) -> Cesta_BasicaIfood_CE

cv<-function(x){coef<-sd(x)/mean(x)*100 
return(coef)}

Cesta_BasicaIfood_CE %>%
...
# Cesta_BasicaIfood_CE %>%
#   dplyr::group_by(produto_base) %>%
#   dplyr::mutate(teste2 = ifelse(produto_base == 'Carne' & preco_cesta >= median(preco_cesta)*1.2 & preco_cesta <= median(preco_cesta)*3.8, 'FALSE',
#                                 ifelse(produto_base == 'Banana' & preco_cesta >= median(preco_cesta)*3.9 & preco_cesta <= median(preco_cesta)*10.8, 'FALSE',
#                                        ifelse(produto_base == 'Açúcar' & preco_cesta >= median(preco_cesta)*0.4 & preco_cesta <= median(preco_cesta)*1.1, 'FALSE',
#                                               ifelse(produto_base == 'Tomate' & preco_cesta >= median(preco_cesta)*0.2 & preco_cesta <= median(preco_cesta)*2.3, 'FALSE',
#                                                      ifelse(produto_base == 'Pão' & preco_cesta >= median(preco_cesta)*0.9 & preco_cesta <= median(preco_cesta)*2.8, 'FALSE',
...
#   dplyr::left_join(DIEESE_CE_total, by = 'mes') %>%
#   dplyr::mutate(diferenca = (preco_calculado - preco_oficial)/preco_oficial) -> Cesta_Final_CE
...
Cesta_BasicaIfood_CE %>%
  ...
  dplyr:: select(mes, preco_calculado, variancia, desvio_padrao, coef_var, 
                 lim_inf, lim_sup, preco_oficial, diferenca)-> Cesta_Final_CE

openxlsx::write.xlsx(Cesta_Final_CE, file='Cesta_BasicaIfood_CE.xlsx')

# tmp %>%
#   dplyr::filter(teste2 == 'TRUE',
#                 mes == '202403',
#                 preco_cesta > preco_oficial) %>%
#   dplyr::select(produto_base) %>%
#   unique()
