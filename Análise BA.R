...
'C:\\Users\\marcu\\Documents\\Tese\\Script\\Bases Ifood\\BA' -> diretorio
...
for(i_arq in 1:length(arquivos)) {
  ...
  rbind(Base_BA, res) -> Base_BA
}

Base_BA2 <- data.frame()

for(i_arq in 216:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_BA2, res) -> Base_BA2
}
...
Base_BA2 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                localidade = dia, dia = hora, hora = X) -> Base_BA2

Base_BA3 <- data.frame()

for(i_arq in 218:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_BA3, res) -> Base_BA3
}
...
Base_BA4 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_BA4
...
Base_BA6 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_BA6

Base_BA7 <- data.frame()

for(i_arq in 302:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_BA7, res) -> Base_BA7
}
...
Base_BA8 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_BA8

Base_BA9 <- data.frame()

for(i_arq in 307:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_BA9, res) -> Base_BA9
}

Base_BA10 <- data.frame()

for(i_arq in 312:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_BA10, res) -> Base_BA10
}
...
Base_BA10 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_BA10


Base_BA11 <- data.frame()

for(i_arq in 313:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_BA11, res) -> Base_BA11
}

Base_BA12 <- data.frame()

for(i_arq in 320:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_BA12, res) -> Base_BA12
}
...
Base_BA12 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_BA12

Base_BA13 <- data.frame()

for(i_arq in 321:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_BA13, res) -> Base_BA13
}

Base_BA14 <- data.frame()

for(i_arq in 326:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_BA14, res) -> Base_BA14
}
...
Base_BA14 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_BA14

Base_BA15 <- data.frame()

for(i_arq in 327:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_BA15, res) -> Base_BA15
}

Base_BA16 <- data.frame()

for(i_arq in 340:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_BA16, res) -> Base_BA16
}
...
Base_BA16 %>%
  ...
  dplyr::rename(produto_base = nome_produto,
                ...
                preco_produto = nome_estabelecimento, 
                nome_estabelecimento = localidade, 
                localidade = dia, dia = hora, hora = X) -> Base_BA16

Base_BA17 <- data.frame()

for(i_arq in 341:length(arquivos)) {
  res <- read.csv2(arquivos[i_arq])
  rbind(Base_BA17, res) -> Base_BA17
}
...
rbind(Base_BA, Base_BA2) -> Base_total_BA
rbind(Base_total_BA, Base_BA3) -> Base_total_BA
rbind(Base_total_BA, Base_BA4) -> Base_total_BA
rbind(Base_total_BA, Base_BA5) -> Base_total_BA
rbind(Base_total_BA, Base_BA6) -> Base_total_BA
rbind(Base_total_BA, Base_BA7) -> Base_total_BA
rbind(Base_total_BA, Base_BA8) -> Base_total_BA
rbind(Base_total_BA, Base_BA9) -> Base_total_BA
rbind(Base_total_BA, Base_BA10) -> Base_total_BA
rbind(Base_total_BA, Base_BA11) -> Base_total_BA
rbind(Base_total_BA, Base_BA12) -> Base_total_BA
rbind(Base_total_BA, Base_BA13) -> Base_total_BA
rbind(Base_total_BA, Base_BA14) -> Base_total_BA
rbind(Base_total_BA, Base_BA15) -> Base_total_BA
rbind(Base_total_BA, Base_BA16) -> Base_total_BA
rbind(Base_total_BA, Base_BA17) -> Base_total_BA

# Ajustando a base sem dados relevantes

Base_total_BA %>%
  dplyr::filter(!(hora == "")) -> Base_total_BA_ajust

# Ajustando os dias do ano para um único padrão: DD/MM/AAAA

unique(Base_total_BA_ajust$dia) -> f
...
Base_total_BA_ajust$dia <- plyr::mapvalues(Base_total_BA_ajust$dia,
                                           from = f,
                                           to = t)
...
SOAR::Store(Base_total_BA_ajust, lib = R_Tese)
...
Base_total_BA_ajust$teste <- apply(sapply(array , grepl, Base_total_BA_ajust$nome_produto), 1, any)
...
Base_total_BA_ajust$preco_produto <- as.numeric(Base_total_BA_ajust$preco_produto)
                                                                        ...
                                                                        ifelse(produto_base == 'Farinha de mandioca', preco*3,
                                                                               ifelse(produto_base == 'Tomate kg', preco*12,
                                                                                      ifelse(produto_base == 'Café', preco*0.3,
                                                                                             ifelse(produto_base == 'Banana kg', preco_duzia*7.5, preco*4.5))))))))))) -> Base_BA_reg

# Armazenando a base

SOAR::Store(Base_BA_reg, lib = R_Tese)
...
Base_BA_reg$mes <- as.character(Base_BA_reg$mes)
...
Base_BA_reg %>%
  dplyr::filter(mes != '202303',
                ...
                !(is.na(preco_cesta)),
                preco_cesta != 0) -> Base_BA_reg

Base_BA_reg$produto_base <- plyr::mapvalues(Base_BA_reg$produto_base,
...
DIEESE %>%
  dplyr::filter(UF == 'BA') %>%
  dplyr::select(-UF) %>%
  as.data.frame() -> DIEESE_BA

DIEESE_BA %>%
  dplyr::filter(produto_base == 'Total') %>%
  dplyr::select(-produto_base) -> DIEESE_BA_total

Base_BA_reg %>%
  dplyr::left_join(DIEESE_BA, by = c('produto_base', 'mes')) -> Cesta_BasicaIfood_BA

cv<-function(x){coef<-sd(x)/mean(x)*100 
return(coef)}

Cesta_BasicaIfood_BA %>%
...
# Cesta_BasicaIfood_BA %>%
#   dplyr::group_by(produto_base) %>%
#   dplyr::mutate(teste2 = ifelse(produto_base == 'Carne' & preco_cesta >= median(preco_cesta)*1 & preco_cesta <= median(preco_cesta)*2.5, 'FALSE',
#                                 ifelse(produto_base == 'Banana' & preco_cesta >= median(preco_cesta)*1.9 & preco_cesta <= median(preco_cesta)*6.5, 'FALSE',
#                                        ifelse(produto_base == 'Açúcar' & preco_cesta >= median(preco_cesta)*0.3 & preco_cesta <= median(preco_cesta)*0.9, 'FALSE',
#                                               ifelse(produto_base == 'Tomate' & preco_cesta >= median(preco_cesta)*1 & preco_cesta <= median(preco_cesta)*3.7, 'FALSE',
#                                                      ifelse(produto_base == 'Pão' & preco_cesta >= median(preco_cesta)*0.5 & preco_cesta <= median(preco_cesta)*3.2, 'FALSE',
#                                                             ifelse(produto_base %in% c('Arroz', 'Feijão', 'Farinha', 'Café', 'Óleo', 'Manteiga', 'Leite') & preco_cesta >= median(preco_cesta)*0.5 & preco_cesta <= median(preco_cesta)*1.5, 'FALSE', 'TRUE'))))))) %>%
#   dplyr::ungroup() %>%
#   dplyr::filter(teste2 == 'FALSE') %>%
...
#   dplyr::left_join(DIEESE_BA_total, by = 'mes') %>%
#   dplyr::mutate(diferenca = (preco_calculado - preco_oficial)/preco_oficial) -> Cesta_Final_BA
...
Cesta_BasicaIfood_BA %>%
  ...
  dplyr:: select(mes, preco_calculado, variancia, desvio_padrao, coef_var, 
                 lim_inf, lim_sup, preco_oficial, diferenca)-> Cesta_Final_BA

openxlsx::write.xlsx(Cesta_Final_BA, file='Cesta_BasicaIfood_BA.xlsx')
